...
mx6q_gpmi_nand_platform_data __initconst = {
	...
	.partitions	             = mx6q_var_som_partitions,
...
static void pcie_3v3_power(void)
{
	...
	gpio_request(VAR_SOM_PCIE_PWR_EN, "pcie_3v3_en");
	gpio_direction_output(VAR_SOM_PCIE_PWR_EN, 0);
	mdelay(10);
	/* enable PCIE_3V3 again */
	gpio_set_value(VAR_SOM_PCIE_PWR_EN, 1);
	gpio_free(VAR_SOM_PCIE_PWR_EN);
}

static void pcie_3v3_reset(void)
{
	/* reset miniPCIe */
	gpio_request(VAR_SOM_PCIE_RST_B_REVB, "pcie_reset_rebB");
	gpio_direction_output(VAR_SOM_PCIE_RST_B_REVB, 0);
	...
	mdelay(1);
	gpio_set_value(VAR_SOM_PCIE_RST_B_REVB, 1);
	gpio_free(VAR_SOM_PCIE_RST_B_REVB);
}
#endif
...
{
	if (0 == enable) {
		gpio_set_value(VAR_SOM_WL1271_WL_EN, 0);		/* momentarily disable */
		mdelay(2);
		gpio_set_value(VAR_SOM_WL1271_WL_EN, 1);
	}
}
...
static int init_wlan(void)
{
	...
	mdelay(2);
	gpio_set_value(VAR_SOM_WL1271_WL_EN, 0);
 ...
 */
MACHINE_START(VAR_SOM_MX6, "Variscite i.MX 6Quad/DualLite/Solo SOM Board")
	/* Maintainer: Uri Yosef <uri.y@variscite.com>  */
